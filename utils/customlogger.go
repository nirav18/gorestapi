package utils

import (
	"os"
	"time"

	"github.com/gin-gonic/gin"
	"github.com/rs/zerolog"
	"github.com/rs/zerolog/log"
	"github.com/rs/zerolog/pkgerrors"
)

// DefaultStructuredLogger logs a gin HTTP request in JSON format. Uses the
// default logger from rs/zerolog.
func DefaultStructuredLogger() gin.HandlerFunc {
	return StructuredLogger(&log.Logger)
}

// StructuredLogger logs a gin HTTP request in JSON format. Allows to set the
// logger for testing purposes.
func StructuredLogger(logger *zerolog.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		start := time.Now() // Start timer)
		path := c.Request.URL.Path
		raw := c.Request.URL.RawQuery
		// Process request
		c.Next()
		// Fill the params
		param := gin.LogFormatterParams{}
		param.TimeStamp = time.Now() // Stop timer
		param.Latency = param.TimeStamp.Sub(start)
		if param.Latency > time.Minute {
			param.Latency = param.Latency.Truncate(time.Second)
		}
		param.ClientIP = c.ClientIP()
		param.Method = c.Request.Method
		param.StatusCode = c.Writer.Status()
		param.ErrorMessage = c.Errors.ByType(gin.ErrorTypePrivate).String()
		param.BodySize = c.Writer.Size()
		if raw != "" {
			path = path + "?" + raw
		}
		param.Path = path
        zerolog.ErrorStackMarshaler = pkgerrors.MarshalStack  // this will enable stack trace 
        file, err := os.OpenFile("myapp.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0664, )
        if err != nil {
            panic(err)
        }
        defer file.Close()
		// Log using the params
		// var logEvent *zerolog.Event
		// var logFileEvent *zerolog.Logger
		if c.Writer.Status() >= 500 {
			// logEvent = logger.Error().Stack()
			// logFileEvent.Level(3) // ErrorLevel = 3
		} else {
			// logEvent = logger.Info()
			// logFileEvent.Level(1) // InfoLevel = 1
		}
		// logEvent.Str("client_id", param.ClientIP).
		// 	Str("method", param.Method).
		// 	Int("status_code", param.StatusCode).
		// 	Int("body_size", param.BodySize).
		// 	Str("path", param.Path).
		// 	Str("latency", param.Latency.String()).
		// 	Msg(param.ErrorMessage)

		// fileLogger := &lumberjack.Logger{
		// 	Filename:   "wikipedia-demo.log",
		// 	MaxSize:    5, //
		// 	MaxBackups: 10,
		// 	MaxAge:     14,
		// 	Compress:   true,
		// }
		// output := zerolog.MultiLevelWriter(os.Stderr, fileLogger)
        zerolog.New(file).Level(1).With(). 
            Str("client_id", param.ClientIP).
			Str("method", param.Method).
			Int("status_code", param.StatusCode).
			Int("body_size", param.BodySize).
			Str("path", param.Path).
			Str("latency", param.Latency.String()).
            Str("message", param.ErrorMessage).Logger()
	}
}

